...
def getProfile(biscuit,site,server_group,web_service,web_application):
    logging.info('Getting Profile.')
    r = requests.get(host+'/SecureSphere/api/v1/conf/webProfile/'+site+'/'+server_group+'/'+web_service+'/'+web_application, headers=headers, cookies=biscuit,verify=False)
    if (r.status_code == 200):
      app_profile = r.json()
      logging.info('Retrieved the Web Profile successfully.')
      logging.debug(str(app_profile))
      return app_profile
    else:
      logging.error('An error occured. Status code is '+r.status_code)
...
web_service=ws['web-services'][0]
web_apps=getAllWebApplications(cookie,site,sg,web_service)
logging.debug('The web application is '+web_apps['webApplications'][0])
web_app = web_apps['webApplications'][0]
profile = getProfile(cookie,site,sg,web_service,web_app)
fname="profile"+web_app+".json"
logging.debug('Writing profile to '+fname+' file')
f=open(fname,"w+")
json_res = json.dumps(profile, indent=4, sort_keys=False)
f.write(json_res)
f.close
logout(cookie)
